...
object UserClient extends IOApp.Simple {
  ...
  /** Given a client and a source stream of Users:-
   * <ol>
   *   <li>Call the [[postUsers]] method to post them to the server</li>
   *   <li>Call the [[getUser]] method fetch all the users we have just created</li>
   *   <li>Finally, call the [[deleteUser]] method to delete all the users we have created</li>
   * </ol>
   * <p>Note that the operations all happen in parallel</p>
   * @param client
   ...
   */
  private def processPostErrors: Pipe[IO, Either[(String, User), User], Nothing] = {
    _.filter(_.isLeft)
      .map {
        case Left((e, u)) => (e,u)
        case Right(u)     => ("",User(-1L,"",""))
      }
      .evalTap( (error,user) => log.error(s"Error posting user: $user - Error: $error"))
      .drain
  }

  /** Build an EmberClient (with an attached [[RateLimitRetry]] policy) and pass it to the [[userClient]] method.
    * @param source
   ...
   * @return The user wrapped in an Option, or None if not found
   */
  private def getUser(id: USER_ID, client: Client[IO]): IO[Option[User]] = {
    val target = uri"http://localhost:8085/api/user" / id
    client.expect[User](target).map(u => Some(u)).handleError { _ => none[User] }
  }
  ...
  private def deleteUser(id: USER_ID, client: Client[IO]): IO[String] = {
    def req(id: USER_ID): Request[IO] = Request[IO](method = Method.DELETE, uri"http://localhost:8085/api/user" / id)
  ...
  private def postUser(user: User, client: Client[IO]): IO[Either[(String, User), User]] = {
    def req(user: User): Request[IO] = Request[IO](method = Method.POST, uri"http://localhost:8085/api/user")
      .withEntity(user)
  ...
  /** Given a [[net.martinprobson.example.common.Source]] stream of [[net.martinprobson.example.common.model.User]]'s,
    ...
    *   The source stream of Users.
    * @return
    *   A result stream containing an Either giving the result of each post call.
    */
